...
(function() {
    ...
    const DEFAULT_CONFIG = {
        ...
        autoAnswerPopup: {
            enabled: true, // 默认开启自动解析弹窗
            autoClose: true, // 默认开启定时关闭
            autoCloseDelay: 3000 // 默认3秒自动关闭
        },
        ai: {
            provider: 'deepseek',
    ...
    async function handleAutoAnswer(problem) {
        ...
        try {
            ...
            if (parsedAnswer) {
                ...
                // 显示自动作答成功弹窗
                if (config.autoAnswerPopup.enabled) {
...
- 主观题请给出完整回答`;
    ...
    // 显示自动作答成功弹窗
    function showAutoAnswerPopup(problem, aiAnswer) {
        // 检查是否已存在弹窗，避免重复显示
        const existingPopup = document.getElementById('ykt-auto-answer-popup');
        if (existingPopup) {
        ...
        const questionText = formatProblemForDisplay(problem);
        ...
        popup.innerHTML = `
            ...
            <div class="popup-body">
                <div class="popup-question">
                    ...
                    <div class="content">${questionText.replace(/\n/g, '<br>')}</div>
                </div>
                <div class="popup-answer">
                    <div class="label">AI回答：</div>
                    <div class="content">${aiAnswer.replace(/\n/g, '<br>')}</div>
                </div>
            </div>
        `;
        ...
        // 如果启用了自动关闭，设置定时器
        if (config.autoAnswerPopup.autoClose) {
            setTimeout(() => {
                if (popup.parentNode) {
                    popup.remove();
                }
            }, config.autoAnswerPopup.autoCloseDelay);
        }
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    try {
                        console.log('[雨课堂助手] API响应状态:', response.status);
                        console.log('[雨课堂助手] API响应内容:', response.responseText);
                        ...
                        if (data.error) {
                            ...
                            console.log('[雨课堂助手] AI回答:', aiResponse);
                            ...
                            console.error('[雨课堂助手] API返回格式异常:', data);
                            reject(new Error('API返回结果格式异常'));
                        }
                    } catch (e) {
                        console.error('[雨课堂助手] 解析API响应失败:', e);
                        reject(new Error(`解析API响应失败: ${e.message}`));
                    }
                },
                onerror: function(error) {
                    console.error('[雨课堂助手] 网络请求失败:', error);
    ...
    function createSettingsPanel() {
            ...
            <div class="panel-body">
                <div class="settings-content">
                    ...
                    <div class="setting-group">
                        ...
                        <div class="setting-item">
                            <label class="checkbox-label">
                                <input type="checkbox" id="auto-answer-popup" ${config.autoAnswerPopup.enabled ? 'checked' : ''}> 
                                <span class="checkmark"></span>
                                启用自动作答成功弹窗
                            </label>
                        </div>
                        <div class="setting-item">
                            <label class="checkbox-label">
                                <input type="checkbox" id="auto-answer-popup-auto-close" ${config.autoAnswerPopup.autoClose ? 'checked' : ''}> 
                                <span class="checkmark"></span>
                                启用弹窗自动关闭
                            </label>
                        </div>
                        <div class="setting-item">
                            <label for="auto-close-delay">弹窗自动关闭延迟 (秒):</label>
                            <input type="number" id="auto-close-delay" min="1" max="30" value="${config.autoAnswerPopup.autoCloseDelay / 1000}">
                            <small>弹窗显示多长时间后自动关闭</small>
                        </div>
                    </div>
        ...
        panel.querySelector('#save-settings').addEventListener('click', () => {
            ...
            const autoAnswerPopup = panel.querySelector('#auto-answer-popup').checked;
            const autoAnswerPopupAutoClose = panel.querySelector('#auto-answer-popup-auto-close').checked;
            const autoCloseDelay = parseInt(panel.querySelector('#auto-close-delay').value) * 1000;
            ...
            config.autoAnswerPopup.enabled = autoAnswerPopup;
            config.autoAnswerPopup.autoClose = autoAnswerPopupAutoClose;
            config.autoAnswerPopup.autoCloseDelay = autoCloseDelay;
        ...
        panel.querySelector('#reset-settings').addEventListener('click', () => {
            if (confirm('确定要重置为默认设置吗？')) {
                ...
                panel.querySelector('#auto-answer-popup').checked = DEFAULT_CONFIG.autoAnswerPopup.enabled;
                panel.querySelector('#auto-answer-popup-auto-close').checked = DEFAULT_CONFIG.autoAnswerPopup.autoClose;
                panel.querySelector('#auto-close-delay').value = DEFAULT_CONFIG.autoAnswerPopup.autoCloseDelay / 1000;
    ...
    function addStyles() {
        GM_addStyle(`
            ...
            .auto-answer-popup .popup-header h4 {
                margin: 0;
                font-size: 16px;
                display: flex;
                align-items: center;
            ...
            .auto-answer-popup .close-btn:hover {
                background: rgba(255,255,255,0.2);
            }

            .auto-answer-popup .popup-body {
                padding: 15px;
                max-height: 500px;
                overflow-y: auto;
            }

            .auto-answer-popup .popup-question,
            .auto-answer-popup .popup-answer {
                margin-bottom: 15px;
            }

            .auto-answer-popup .popup-question:last-child,
            .auto-answer-popup .popup-answer:last-child {
                margin-bottom: 0;
            }

            .auto-answer-popup .label {
                font-weight: bold;
                color: #333;
                margin-bottom: 8px;
                display: flex;
            ...
            .auto-answer-popup .label::before {
                content: "";
                width: 4px;
                height: 16px;
                background: #4caf50;
            ...
            .auto-answer-popup .popup-answer .content {
                background: #e8f5e8;
