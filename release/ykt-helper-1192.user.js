...
(function() {
 ...
 // 强制指定
  ...
  function getSlidesDocument() {
    // 1) 先看当前 document 自己有没有 #content-page-wrap
    if (document.querySelector("#content-page-wrap")) return document;
    // 2) 遍历所有同源子 frame，找到有 #content-page-wrap 的那个
      ...
      // 跨域 frame 会在这里抛异常，忽略即可
    }
    // 3) 找不到就退回当前 document（避免崩）
  ...
  function collectStaticSlideURLsFromDom() {
    ...
    // 然后再用宽松规则多抓一些（避免只抓 active 那一页）
    // 里面也会用 getSlidesDocument
        const doc = getSlidesDocument();
  ...
  /**
   * 将静态 slide URL 转成 presentation + slides，并注入 repo
   * 返回是否发生了注入/更新
 ...
 // 放宽一点重试次数，但仍防止刷屏
      ...
      // 优先监听 slides-list 容器，如果还不存在就先监听 body
