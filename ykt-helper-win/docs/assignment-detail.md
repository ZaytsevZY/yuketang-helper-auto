...
每道题下方提供独立的“AI解释（试用）”，只将所点击题目的题干、选项、个人作答及批改反馈整理为文本，不混入其他题目或整卷说明；AI 对话标题保留原题号，并跳转 AI 页预填可编辑输入框。点击“发送解释请求”才调用当前配置的 AI；进入页面不触发“自动分析最新课堂题”，请求不携带课堂题目 ID、课件图或网页截图，不进入提交答案流程。支持继续追问和新对话。

这只是原始文本提取，不是字体解码：保留置换 Unicode，标记加密片段；有字体 URL 但无片段标记时也保留未解码提示。图片仅保留占位符，公式保留已有文本形式，不能声称取得图中的内容。按用户新的试用要求允许检查、编辑并主动发送该提取结果，尚未实现可靠解码、OCR 或纯文本导出。字符与字形的边界见 [字体规范](font-policy.md)。
