...
const BASE_SYSTEM_PROMPT = [
 '你是 Kimi，由 Moonshot AI 提供的人工智能助手。你需要在以下规则下工作：',
  ...
  '3) 若识别不到明确题目，且用户没有主动提出题目型问题，则输出：',
  '   STATE: NO_QUESTION',
  '   SUMMARY: <用一句话概括页面/上下文的主要内容>',
  '4) 若存在明确题目，则输出以下格式的内容：',
  '   单选：格式要求：\n答案: [单个字母]\n解释: [选择理由]\n\n注意：只选一个，如A',
  '   多选：格式要求：\n答案: [多个字母用顿号分开]\n解释: [选择理由]\n\n注意：格式如A、B、C',
  '   投票：格式要求：\n答案: [单个字母]\n解释: [选择理由]\n\n注意：只选一个选项',
  '   填空/主观题/用户的问题：答案: [直接给出答案内容]，解释: [简要说明]',
  '5) 避免臆测。无法确定时，按 NO_QUESTION 给出 SUMMARY。',
].join('\n');
...
const VISION_GUIDE = [
  ...
  'C. 再给出 TYPE/ANSWER/RATIONALE；',
  'D. 若不是题目，按 NO_QUESTION 输出，并给出 SUMMARY。',
].join('\n');
...
export async function queryKimiVision(imageBase64, textPrompt, aiCfg) {
    ...
    const visionTextHeader = [
      ...
      '请严格先执行“是否有题目”的决策闸，再决定输出模板（HAS_QUESTION 或 NO_QUESTION）。',
      VISION_GUIDE
    ].join('\n');
