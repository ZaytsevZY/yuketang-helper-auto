...
(function() {
 ...
 // ★ 用页面 window
  ...
  var tpl$4 = '<div id="ykt-ai-answer-panel" class="ykt-panel">\n  <div class="ykt-panel-header">\n    <h3><i class="fas fa-robot"></i> AI 融合分析</h3>\n    <button id="ykt-ai-close" class="ykt-btn-close">&times;</button>\n  </div>\n  <div class="ykt-panel-content">\n    <div class="ykt-form-group">\n      <label>当前题目：</label>\n      <div id="ykt-ai-question-display" class="ykt-question-display">系统将自动识别当前页面的题目</div>\n      <div id="ykt-ai-text-status" class="text-status">正在检测题目信息...</div>\n      <small>提示：系统使用融合模式，同时分析题目文本信息和页面图像，提供最准确的答案。</small>\n    </div>\n    \n    <div class="ykt-form-group">\n      <button id="ykt-ai-ask" class="ykt-btn ykt-btn-primary">\n        <i class="fas fa-brain"></i> 融合模式分析（文本+图像）\n      </button>\n    </div>\n\n    <div id="ykt-ai-loading" class="ykt-loading" style="display: none;">\n      <i class="fas fa-spinner fa-spin"></i> AI正在使用融合模式分析...\n    </div>\n    \n    <div id="ykt-ai-error" class="ykt-error" style="display: none;"></div>\n    \n    <div class="ykt-form-group">\n      <label>AI 分析结果：</label>\n      <div id="ykt-ai-answer" class="ykt-ai-answer"></div>\n    </div>\n  </div>\n</div>';
  // src/ai/kimi.js
  /**
  ...
  // 改进的融合模式 prompt 格式化函数
    ...
    let basePrompt = hasTextInfo ? `请结合提供的文本信息和图片内容分析以下${problemType}，并严格按照格式要求回答。` : `请仔细观察图片内容，识别并分析${problemType}，严格按照格式要求回答。`;
 ...
 // 单选题
  ...
  // 内部自动答题处理函数 - 融合模式（文本+图像）
    ...
    try {
      ...
      // 截图
            const imageBase64 = await captureProblemForVision();
      if (!imageBase64) return ui.toast("无法截取页面图像，跳过自动作答", 3e3);
      // 使用新的 formatProblemForVision 函数构建提示
      ...
      if (!parsed) return ui.toast("融合模式无法解析答案，跳过自动作答", 3e3);
      await submitAnswer(problem, parsed);
      actions.onAnswerProblem(problem.problemId, parsed);
      ui.toast(`融合模式自动作答完成: ${String(problem.body || "").slice(0, 30)}...`, 3e3);
      showAutoAnswerPopup(problem, aiAnswer);
    } catch (e) {
      console.error("[AutoAnswer] 融合模式失败", e);
      ui.toast(`融合模式自动作答失败: ${e.message}`, 3e3);
    }
  }
  const actions = {
