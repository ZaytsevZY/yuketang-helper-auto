...
> [!WARNING]
> 当前只是基于 Android SDK 完成的客户端开发原型，状态为“待测试”，不是正式发布版本。现阶段仅确认可以编译、通过本地单元测试并在 macOS ARM64 模拟器启动；真实账号登录、真实课堂、WebSocket 推题、模型调用、答案提交及不同品牌真机兼容性尚未完成端到端验证。请勿直接用于重要课堂。

开发分支：`feat/dev_android_main`。详细验证范围和测试记录要求见 [`TESTING.md`](./TESTING.md)。
...
以上条目描述的是当前代码目标和已实现接口，不代表相关功能已经在真实雨课堂环境验证成功。
...
首次测试请使用非重要课堂和可撤销的测试数据，并按 [`TESTING.md`](./TESTING.md) 逐项记录结果。
